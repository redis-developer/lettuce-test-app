...
public class ExtendedLoggingMeterRegistry extends StepMeterRegistry {

    private static final InternalLogger log = InternalLoggerFactory.getInstance(ExtendedLoggingMeterRegistry.class);
    ...
    public ExtendedLoggingMeterRegistry() {
     ...
     */
    public ExtendedLoggingMeterRegistry(LoggingRegistryConfig config, Clock clock) {
     ...
     * @since 1.11.0
     */
    public ExtendedLoggingMeterRegistry(Consumer<String> loggingSink) {
     ...
     * @since 1.11.0
     */
    public ExtendedLoggingMeterRegistry(LoggingRegistryConfig config, Clock clock, Consumer<String> loggingSink) {
        this(config, clock, new NamedThreadFactory("logging-metrics-publisher"), loggingSink, null);
    }

    private ExtendedLoggingMeterRegistry(LoggingRegistryConfig config, Clock clock, ThreadFactory threadFactory,
            ...
            getMeters().stream().sorted((m1, m2) -> {
                ...
                m.use(gauge -> loggingSink.accept(print.id() + " value=" + print.value(gauge.value())), counter -> {
                    ...
                    if (!config.logInactive() && count == 0)
                        return;
                    loggingSink.accept(print.id() + " throughput=" + print.rate(count) + " count=" + count);
                }, timer -> {
    ...
    public static class Builder {
        ...
        public ExtendedLoggingMeterRegistry build() {
            return new ExtendedLoggingMeterRegistry(config, clock, threadFactory, loggingSink, meterIdPrinter);
        }
